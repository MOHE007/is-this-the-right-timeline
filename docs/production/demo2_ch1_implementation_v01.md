...
本规格只约束实现接口和验收行为。水墨画面、角色造型和 UI 视觉由梁博森主导；剧情文本与分支由 Codex 维护，Marvis 审校后再冻结。当前占位 `main.gd` 已支持 content/state/manifest 加载、命名与分组条件过滤、结局路由，以及同场景终止选择后继续后续 beat；真实资源装载、保存点和问答适配仍待 DSH Desktop 接入。
